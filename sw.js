const CACHE='maurimove-v9';
const APP=['./','./index.html','./manifest.webmanifest','./icon.svg','./route-fix.js?v=8','./route-fix-v9.js','./data/transit/route-57.json'];
...
self.addEventListener('fetch',e=>{
 ...
 if(u.origin===location.origin && (u.pathname.endsWith('/')||u.pathname.endsWith('/index.html'))){
  e.respondWith(fetch(e.request).then(async r=>{
   ...
   const patched=text.replace(/<script[^>]+src=["']\.\/route-fix(?:-v7)?\.js(?:\?[^"']*)?["'][^>]*><\/script>/gi,'').replace(/<script[^>]+src=["']\.\/route-fix-v9\.js[^"']*["'][^>]*><\/script>/gi,'').replace('</body>','<script src="./route-fix.js?v=8"></script><script src="./route-fix-v9.js"></script></body>');
