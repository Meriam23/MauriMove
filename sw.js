const CACHE='maurimove-v4';
const APP=['./','./index.html','./manifest.webmanifest','./icon.svg','./route-fix.js','./data/transit/route-57.json'];
...
self.addEventListener('fetch',e=>{
 ...
 if(u.origin===location.origin && (u.pathname.endsWith('/')||u.pathname.endsWith('/index.html'))){
  e.respondWith(fetch(e.request).then(async r=>{
   ...
   const patched=text.includes('route-fix.js')?text:text.replace('</body>','<script src="./route-fix.js?v=4"></script></body>');
