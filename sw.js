const CACHE='maurimove-v7';
const APP=['./','./index.html','./manifest.webmanifest','./icon.svg','./route-fix-v7.js','./data/transit/route-57.json'];
...
self.addEventListener('fetch',e=>{
 ...
 if(u.origin===location.origin && (u.pathname.endsWith('/')||u.pathname.endsWith('/index.html'))){
  e.respondWith(fetch(e.request).then(async r=>{
   ...
   const patched=text.replace(/<script[^>]+src=["']\.\/route-fix(?:-v7)?\.js(?:\?[^"']*)?["'][^>]*><\/script>/gi,'').replace('</body>','<script src="./route-fix-v7.js"></script></body>');
